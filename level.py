...
class Level:
    ...
    def setup(self):
        ...
        #collision titles
        for x, y, surf in tmx_data.get_layer_by_name('Collision').titles():
            Generic((x * TILE_SIZE, y * TILE_SIZE), pygame.Surface((TILE_SIZE, TILE_SIZE)), self.collision_sprites)

        # Player      
        for obj in tmx_data.get_layer_by_name('Player'):
            if obj.name == 'Start':
                self.player = Player((obj.x, obj.y), self.all_sprites, self.collision_sprites) #posição e grupo do player
        Generic(
            pos = (0,0),
